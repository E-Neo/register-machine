;;; register_machine.scm --- register machine model

;; Copyright (C) 2016 E-Neo
;; Most of the codes in this file comes from the famous textbook
...
(define-module (register-machine)
  #:export (make-machine
            start
...
(define (make-register)
  (let ((contents '*unsigned*))
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
            ((eq? message 'set)
             (lambda (value) (set! contents value)))
            (else "Unknown request --- REGISTER" message)))
    dispatch))
...
;; execution procedures for instructions
...
;;; register_machine.scm ends here
